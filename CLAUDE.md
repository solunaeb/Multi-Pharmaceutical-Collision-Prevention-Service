...
```
├── CLAUDE.md              # 마스터 지침서 (이 파일)
├── TASKS.md               # Phase별 개발 작업 목록 (Phase 0~4, 체크박스 관리)
├── README.md              # 프로젝트 셋업 가이드 (환경 구성, 실행 방법)
...
**작업 순서는 TASKS.md를 따른다.** 작업 전 반드시 관련 문서를 읽고 시작할 것. 문서 간 충돌이 있으면 이 CLAUDE.md의 내용을 우선한다.
